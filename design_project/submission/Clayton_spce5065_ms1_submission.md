...
![ORCA conceptual diagram: the servicer docking with a client satellite](figures/fig0_orca_concept.png)

# ORCA
## GEO Repair and Cooperative Assist Servicing Tug
...
| ORCA | Orbital Repair and Cooperative Assist (this system) |
...
The overall constraints are a servicer life of at least 5 years, servicing multiple clients in sequence, within a $100M budget. These objectives drive the derived requirements carried into the environment analysis: precision RPO sensors (cameras and LIDAR), a docking mechanism and robotic arm, generous stationkeeping propellant, and avionics hardened against the charging and radiation environment. Per the milestone directions, the orbit is not chosen until after the hazard analysis (Sections 2 through 4).

---

## 2. Sun-Earth System and Risks to Satellite Operations (GEO and LEO)

The Sun-Earth system is a coupled environment: the Sun emits radiation and plasma, Earth's magnetic field deflects and traps the charged component, and what reaches a satellite depends strongly on its orbit. I analyzed **GEO** (the mission home) and **LEO** (the sharpest contrast).
...
- **Solar radiation pressure (SRP):** photon momentum (~4.5 µN/m² at 1 AU) is negligible for drag but, on ORCA's large arrays, is a real orbital perturbation (an eccentricity oscillation, Section 7) and an attitude torque.
...
For ORCA the charging risk is doubled: an ESD during docking could damage the tug *and* the client.
...
**Cost argument (quantified).** Skipping TVAC to save a small fraction of a $100M program risks the entire asset *plus* the client it is servicing. ORCA cannot repair itself on orbit, so an undetected workmanship or contamination failure is mission-ending: one hazed docking sensor could abort every rendezvous, and a few percent of lost array or radiator performance compounds over a 5-year life. Vacuum testing is cheap insurance against a total loss. (The calculus only flips for a low-cost, high-quantity CubeSat build where a unit is expendable; ORCA is the opposite.)
...
- **The clients live at GEO.** Defunct and low-fuel comsats, national-security assets, and the graveyard (~300 km above GEO) are all in the belt, so ORCA must operate there to rendezvous, dock, refuel, and tow them [1].
...
**Figure 2** simulates the ORCA orbit, propagated numerically over one sidereal day (the GEO period). Panel A is the inertial view, with the satellite stepped hourly to show one revolution per sidereal day; Panel B is the resulting ground track. A stationkept satellite holds a single point over its assigned longitude (105 W here). Without north-south stationkeeping, as luni-solar gravity grows the inclination (Section 7), the ground track opens into the classic figure-eight analemma, reaching roughly ±5° of latitude after a few years and ±15° after about 26.5 years, which is why a GEO servicer must budget stationkeeping propellant to hold its slot.
...
![Figure 3: Drag-decay lifetime vs. altitude for the ORCA ballistic coefficient](figures/fig3_drag_lifetime.png)
...
**Assumptions:** representative tug mass and area, the HW2 density fit for the LEO contrast, a standard GEO density ~$10^{-15}$ kg/m³, and standard luni-solar/triaxiality rates [11], [12]. So ORCA's life is set by propellant, not decay: ~250 m/s over 5 years just to hold station, on top of the servicing propellant.

---

## 8. Conclusions

I selected **GEO** for ORCA (circular, equatorial, 35,786 km altitude) because the servicing clients (defunct and low-fuel comsats, national-security assets, and the graveyard population) live there; the mission is inherently geostationary.

The dominant GEO hazards are **surface and deep-dielectric charging with ESD** (the leading anomaly cause, per Galaxy 15) and **direct energetic-particle radiation** over 5+ years, plus eclipse thermal cycling and solar-radiation-pressure torque on the large arrays; LEO's drag, atomic oxygen, and debris are avoided. Space weather matters mainly for scheduling docking around charging and SEP events and managing the downlink around sun outages, and vacuum testing is essential because contamination on the RPO optics or a cold-welded mechanism would be mission-ending for a vehicle with no self-repair. Drag is not the lifetime limiter at GEO (decay ~$10^{5}$ to $10^{6}$ years); the real driver is ~50 m/s/yr north-south stationkeeping, which sizes the propellant budget.
...
The four figures (cover concept, orbit regimes, orbit propagation, and drag lifetime) and the Section 7 decay numbers were generated by `spce5065_ms1_figs.py` (matplotlib; reuses the HW2 neutral-density drag model). Running `python spce5065_ms1_figs.py` regenerates all figures in `figures/` and prints the LEO decay table and the GEO drag-timescale check.
