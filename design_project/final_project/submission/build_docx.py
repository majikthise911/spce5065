...
SRC = os.path.join(HERE, "Clayton_spce5065_final_submission.md")
OUT = os.path.join(HERE, "Clayton_spce5065_final_submission.docx")
...
# 17 numbered figures plus the cover concept image.
N_IMAGES = 18
...
def verify(docx_path):
    ...
    if not populated:
        print("  NOTE: the TOC field is still unpopulated in this file. Word fills "
              "it on open\n        because word/settings.xml carries "
              "<w:updateFields w:val=\"true\"/>. Open the docx\n        in Word "
              "once and save, or hand in the PDF from build_pdf.py, which has\n"
              "        the table of contents already rendered with page numbers.")
    if entries < 10 and not populated:
        pass   # Word has not run yet; it populates on open via updateFields
    if len(media) != N_IMAGES:
        problems.append("image count is %d, expected %d" % (len(media), N_IMAGES))
...
def main():
    ...
    try:
        cmd = [pandoc, staged, "-o", OUT]
        ref = os.path.join(HERE, "reference.docx")
        if os.path.exists(ref):
            # Block-justified body text, a grammar-rubric line item. See
            # make_reference_docx.py; the markdown's <style> block only
            # reaches the HTML and PDF outputs.
            cmd[1:1] = ["--reference-doc", ref]
