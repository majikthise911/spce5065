"""
Figure 14: solar panel power over one day, from the STK Solar Panel tool.
...
OUT = os.path.join(HERE, "fig14_stk_power.png")
...
MESA_EFFICIENCY = 0.30 * 0.90 * 0.90   # Section 12.2 -> 0.243
EQ15_BOL_W = 3387.0             # Eq. (15)
EQ13_ECLIPSE_MIN = 69.4         # Eq. (13)
...
def main():
    ...
    print("normalised BOL power  : %.1f W   (Eq. 15 says %.0f W)" % (mesa_bol, EQ15_BOL_W))
    print("umbra                 : %.1f min (Eq. 13 says %.1f min)" % (umbra_min, EQ13_ECLIPSE_MIN))
    ...
    ax.axhline(EQ15_BOL_W, color="#d62728", lw=1.2, ls=":",
               label="Eq. (15) BOL, %d W" % EQ15_BOL_W)

    ax.axvspan(shadow[0], shadow[-1], color="#333333", alpha=0.12)
    ax.annotate("eclipse\numbra %.0f min\n(Eq. 13: %.1f min)" % (umbra_min, EQ13_ECLIPSE_MIN),
