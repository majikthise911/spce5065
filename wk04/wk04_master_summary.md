...
1. **Macroscopically neutral** (quasineutral): electron density $\approx$ ion density, $n_e \approx n_i$.
...
**Memory hook:** a gas of independent colliding balls vs a plasma that "votes as a bloc." You only need to ionize about 1 part in a million (ionized fraction $\sim 10^{-6}$) to flip a gas into this collective, conductive behavior.
...
$$\omega_p = \sqrt{\frac{n_e\, e^2}{\varepsilon_0\, m_e}}, \qquad f_p = \frac{\omega_p}{2\pi} \approx 9\sqrt{n_e}\ \text{Hz} \quad (n_e\ \text{in}\ \text{m}^{-3})$$
...
$$v_{\text{mean}} = \sqrt{\frac{8\, k_B\, T}{\pi\, m}}$$
...
| **F2** | 180 to 350 km | **Dominant, peaks near $10^6\ \text{cm}^{-3}$ day** | The main reflector; survives the night because recombination is slowest up high |
...
$$n_{e,\max} = 1.24\times10^{4}\, f^2 \qquad (n_e\ \text{in}\ \text{cm}^{-3},\ f\ \text{in MHz})$$
...
$$V_f = \frac{k_B\, T_e}{e}\,\ln\!\left[\frac{(v_{s/c})\,A_i}{4\,v_{e,\text{th}}\,A_e}\right]$$
...
|---|---|---|
| $V < 0$ | repelled | $I_e \propto \exp\!\left(\dfrac{eV}{k_B T_e}\right)$ (Boltzmann tail, shrinks fast) |
| $V > 0$ | attracted | $I_e \propto \left[1 + \dfrac{eV}{k_B T_e}\right]$ (linear growth) |

Ions follow the mirror-image rule ($V > 0$ repels them, $V < 0$ attracts them). The reference current is $I_o = \tfrac{1}{4}\, e\, n\, v_{\text{mean}}$ times collecting area. **This is exactly the HW4 P5 setup.**

### 0.5 Ionospheric signal delay (HW4 P3)

From Lesson Part 3. Free electrons slow and bend a radio signal. The delay depends on **Total Electron Content (TEC)**, the number of electrons in a $1\ \text{m}^2$ column along the path, and falls off as $1/f^2$.

**Time delay** (standard ionospheric relation shown in Lesson Part 3):

$$\Delta t = 1.345\times10^{-7}\,\frac{\text{TEC}}{f^2} \qquad (\text{seconds; TEC in}\ \text{electrons/m}^2,\ f\ \text{in Hz})$$

**Excess range** (extra apparent distance if you assumed vacuum light speed):

$$\Delta R = 40.3\,\frac{\text{TEC}}{f^2} = c\,\Delta t \qquad (\text{meters})$$
...
- **The transition is cheap:** a neutral gas becomes electrically conductive at an ionized fraction as low as $\sim 10^{-6}$. You do not need much ionization to change the physics entirely.
- **Three conditions for the plasma state:**
  1. **Quasineutrality:** $n_e \approx n_i$ over macroscopic volumes.
  ...
  3. **Debye shielding with many particles per sphere:** the number of charges in a Debye sphere must be $\gg 1$, so the crowd can statistically screen a local charge.
...
$$\alpha_g = \frac{n_i}{n_a} \qquad (\text{ions divided by neutral atoms})$$

- Weakly ionized: $\alpha_g \ll 1$ (fluorescent lamps, ionosphere)
...
- **Temperature in energy units:** average kinetic energy $\sim k_B T$, so plasma temperatures are quoted in eV. Anchor: **1 eV = 11,600 K**; room temperature (300 K) $\approx 0.026$ eV.

### Debye length: how far a charge's voice carries

$$\lambda_D = \sqrt{\frac{\varepsilon_0\, k_B\, T_e}{n_e\, e^2}} \qquad (\text{shielding distance})$$
...
$$N_D = \frac{4}{3}\pi\, n\, \lambda_D^3 \gg 1 \qquad (\text{particles in a Debye sphere})$$

This is the quantitative test for "is it a plasma?": only when $N_D \gg 1$ does collective screening work.
...
$$\omega_p = \sqrt{\frac{n\, e^2}{\varepsilon_0\, m_e}}, \qquad f_p = \frac{\omega_p}{2\pi} \approx 9\sqrt{n}\ \text{Hz} \quad (n\ \text{in}\ \text{m}^{-3})$$

- **Derivation intuition:** displace a slab of electrons off the ions; the exposed charge pulls them back, they overshoot and oscillate, like a mass on a spring. Denser plasma rings at higher frequency. Ions are $\sim 2000\times$ heavier, so they barely move.
...
*Fig 1.2: the plasma "zoo." Densities span ~25 orders of magnitude, temperatures ~7. Earth's ionosphere is cold ($T \le 0.1$ eV) and tenuous; the interplanetary medium is diffuse yet hot (~100 eV).*
...
### Remember this
1. A plasma is a quasineutral, ionized medium that responds *collectively*; the three tests are **quasineutrality, collective behavior, and $N_D \gg 1$**.
2. **Debye length** is the shielding distance: hotter = longer, denser = shorter.
3. **Plasma frequency $f_p \approx 9\sqrt{n}$ Hz** is the electron crowd's oscillation and the reflect/transmit cutoff (why the ionosphere bounces radio).
4. **Degree of ionization** classifies plasmas; conductive behavior starts near $\alpha_g \sim 10^{-6}$, and temperature drives ionization.
...
- **Bow shock** ($\sim 15\ R_E$): standoff shock where the supersonic solar wind first decelerates.
- **Magnetosheath:** turbulent region between bow shock and magnetopause.
- **Magnetopause** ($\sim 10\ R_E$): the true outer boundary, where solar-wind pressure balances the compressed field. High-speed wind compresses it to $\sim 7\ R_E$ (note: **geosynchronous orbit sits at $6.6\ R_E$**, so it can be exposed).
- **Magnetotail:** stretches past the Moon's orbit ($\sim 60\ R_E$).

**Three current systems:**
- **Magnetopause current:** from magnetosheath plasma meeting the geomagnetic field; highly time-variable.
- **Plasma sheet current** (dawn-to-dusk): separates north/south tail-lobe fields. Electrons 0.5 to 1.0 keV, ions 2 to 5 keV, density $\sim 0.5\ \text{cm}^{-3}$.
- **Ring current** ($3$ to $6\ R_E$): ions drift west, electrons east; that separation is the current. Average energy $\sim 85$ keV. This current is what intensifies during magnetic storms.
...
- **Plasmasphere:** cold ionospheric plasma co-rotating with Earth. Density $\sim 10^4\ \text{cm}^{-3}$ at 1000 km, dropping sharply at the **plasmapause**.
- **Van Allen radiation belts:** trapped high-energy electrons and protons, equator out to $\pm 50^\circ$ latitude. The **South Atlantic Anomaly (SAA)** is where the field is locally weak and the belts dip lowest, a major radiation hazard. The **outer belt** ($\sim 1$ MeV) responds to even minor storms; the **inner belt** ($> 25$ MeV) barely reacts.

> **One number to remember:** magnetopause $\sim 10\ R_E$ sunward, magnetotail $\sim 60\ R_E$. Sun squashes, tail stretches.
...
| Layer | Peak alt | $n_{e,\max}$ | Notes |
|---|---|---|---|
| **D** | ~90 km | $1.5\times10^4\ \text{cm}^{-3}$ noon; **absent at night** | Absorbs HF; site of solar-flare blackouts |
| **E** | ~110 km | $1.5\times10^5\ \text{cm}^{-3}$ noon; $<10^4$ night | Fits the Chapman model well |
| **F1** | ~200 km | $2.5\times10^5\ \text{cm}^{-3}$ noon; **absent at night** | Chapman-valid |
| **F2** | ~300 km (variable) | **$10^6$ noon; $10^5$ midnight** | The dominant layer; survives the night |
...
$$n_{e,\max} = 1.24\times10^{4}\, f^2 \qquad (n_e\ \text{in}\ \text{cm}^{-3},\ f = \text{critical frequency in MHz})$$
...
$$\frac{dn_e}{dt} = q_v - \alpha_{\text{eff}}\, n_e^2$$

Electron density scales as $\sqrt{\cos\theta}$ with solar zenith angle $\theta$, driving the day/night and seasonal swing. Valid for E and F1; D and F2 need modified loss terms.
...
### Remember this
1. **Magnetosphere distances:** bow shock $\sim 15\ R_E$, magnetopause $\sim 10\ R_E$ (7 when compressed, below the $6.6\ R_E$ geosync belt), tail $\sim 60\ R_E$.
...
4. **$n_{e,\max} = 1.24\times10^4\, f^2$** links density to plasma/critical frequency; Chapman balance governs the layers.
...
$$v_{th} = \sqrt{\frac{8\, k_B\, T}{\pi\, m}} \qquad (\text{mean thermal speed})$$

- Speed scales with $\sqrt{T/m}$. At the same temperature, a light particle is much faster.
- **The chapter in one number:** in LEO, $T \sim 1000$ K but $m_{\text{ion}}/m_e$ is thousands, so electron thermal speed $\sim 200$ km/s while ion thermal speed is under 1 km/s. Orbital speed is $\sim 8$ km/s, so electrons are $\sim 25\times$ faster than the vehicle and ions are slower than it.
...
$$I_T(V) = I_e - I_i + I_{se} + I_{si} + I_{bse} + I_{ph} + I_b = 0$$
...
$$I_i = e\, n_o\, (v_{s/c})\, A_i \qquad (\text{ram scoop, no exponential})$$

$$I_e = \tfrac{1}{4}\, e\, n_o\, v_{e,\text{th}}\, A_e\, \exp\!\left(\frac{eV}{k_B T_e}\right) \qquad (\text{thermal flux} \times \text{Boltzmann factor})$$
...
|---|---|---|---|
| $V < 0$ | repels electrons, attracts ions | $I_e \sim \exp(eV/k_B T_e)$ | Boltzmann tail, falls off fast |
| $V > 0$ | attracts electrons, repels ions | $I_e \sim [1 + eV/k_B T_e]$ | orbit-limited linear growth |
...
- **Insulators have huge yields** (MgO $\sim 23$) because they cannot bleed off secondary energy. Ion-driven yields are $\sim 0.1$ to $0.3$ for metals, negligible in LEO but significant in GEO.
...
|---|---|---|
| Plasma density | $\sim 10^5\ \text{cm}^{-3}$ | $\sim 1\ \text{cm}^{-3}$ |
| Energies | ~1000 K, few eV | keV (electrons ~2.4 keV, ions ~10 keV) |
| Typical floating potential | $\sim -1$ V | thousands of volts negative |
...
### Remember this
1. **Electrons are far faster than ions**, so an unbiased body charges **negative** to its floating potential; LEO $\sim -1$ V, storm-time GEO to kilovolts.
2. **Everything follows from current balance** (all currents sum to zero); repelled species carry $\exp(eV/k_B T)$, attracted species $[1 + eV/k_B T]$.
...
- **GEO plasma is two populations:** a hot, tenuous thermal plasma ($0.1$ to $1\ \text{cm}^{-3}$, 4 to 10 keV, spiking to 16 to 30 keV in storms) plus the outer radiation belt (electrons 0.1 to 10 MeV). GEO sits inside the outer belt.
...
$$\tau_{\text{decay}} \sim \rho\, \varepsilon_0\, \varepsilon_r$$
...
- **Counterintuitive twist:** radiation *aging* makes polyimide **more** conductive, moving it from "Problem" toward "Safe." **Pristine Kapton is the riskier state** (post-damage conductivity rose $\sim 1000\times$).
...
> **Key numbers:** at peak (strongly southward IMF), $\sim 10^{28}$ solar-wind particles cross the magnetopause per second; flux-transfer events recur about every 8 minutes.
...
- **The loss cone is narrow** (1 to 4 degrees near the equatorial plane): particles must be aimed almost exactly along $B$ to precipitate, or the magnetic mirror reflects them. This is why outflow exceeds precipitation $\sim 10\times$.
...
| Boltzmann, $k_B$ | $1.381\times10^{-23}$ J/K |
| Permittivity, $\varepsilon_0$ | $8.854\times10^{-12}$ F/m |
...
|---|---|---|
| Debye length | $\lambda_D = \sqrt{\dfrac{\varepsilon_0 k_B T_e}{n_e e^2}}$ | shielding distance; HW4 P2 |
| Plasma frequency | $f_p = \dfrac{1}{2\pi}\sqrt{\dfrac{n_e e^2}{\varepsilon_0 m_e}} \approx 9\sqrt{n_e}\ \text{Hz}$ | $n_e$ in $\text{m}^{-3}$; reflect/transmit cutoff |
| Reflection cutoff | $n_{e,\max} = 1.24\times10^{4}\, f^2$ | $n_e$ in $\text{cm}^{-3}$, $f$ in MHz |
| Mean thermal speed | $v_{\text{mean}} = \sqrt{\dfrac{8 k_B T}{\pi m}}$ | electrons $\gg$ ions at equal $T$ |
| Floating potential | $V_f = \dfrac{k_B T_e}{e}\ln\!\left[\dfrac{(v_{s/c}) A_i}{4 v_{e,\text{th}} A_e}\right]$ | comes out negative; HW4 P5 |
| Reference current | $I_o = \tfrac{1}{4} e\, n\, v_{\text{mean}} \times (\text{area})$ | per collecting area |
| Biased electron current | $V<0$: $I_e \propto \exp(eV/k_B T_e)$; $V>0$: $I_e \propto [1 + eV/k_B T_e]$ | ions are the mirror image; HW4 P5 |
| Ionospheric time delay | $\Delta t = 1.345\times10^{-7}\,\dfrac{\text{TEC}}{f^2}$ | seconds; TEC in $\text{e}/\text{m}^2$, $f$ in Hz; HW4 P3 |
| Excess range | $\Delta R = 40.3\,\dfrac{\text{TEC}}{f^2} = c\,\Delta t$ | meters; HW4 P3 |

### Reference numbers to anchor intuition
- LEO floating potential: $\sim -1$ V. GEO storm-time: up to tens of kilovolts.
- LEO plasma density $\sim 10^5\ \text{cm}^{-3}$; GEO $\sim 1\ \text{cm}^{-3}$.
- Ionosphere F2 peak: $\sim 10^6\ \text{cm}^{-3}$ (day), ~300 km.
- Magnetopause $\sim 10\ R_E$; geosync $6.6\ R_E$; magnetotail $\sim 60\ R_E$.
...
| **P3** | Ionospheric time delay and excess range (150 MHz, 1.6 GHz, $\text{TEC}=10^{18}$) | Section 0.5 ($\Delta t$ and $\Delta R$); note the $1/f^2$ dependence |
