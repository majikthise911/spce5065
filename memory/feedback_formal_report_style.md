...
**Three defects found by reading the rendered output, not the source, on 8 to 9 Aug 2026. All three were invisible in the markdown and had survived every earlier print:**

1. **Unescaped `$` in currency silently becomes a math span.** `$100M program risks ... The $9M line` renders everything between the two dollar signs in math italic. Eighteen currency signs across the report were affected, including in the executive summary, the conclusions, and a `Cost ($M)` table header. Escape every one as `\$`. Checking for lines with an *odd* count of `$` catches only some of them: `$100M cap with $5M held` has an even count and is still wrong. Grep for `\$\d` and `\$[A-Za-z]` and inspect each hit.
2. **Figures can end up with a caption but no in-text callout.** The three STK figures had none, because that section was inherited from Milestone 2 where the prose described the results and let the images sit beside them. Formal-paper convention and the grammar rubric line both want a `**Figure N**` reference in the body. Audit by stripping the `![Figure N](...)` image lines and then searching the remainder for each number.
3. **Numbers stated without a basis are the recurring failure mode in this project.** The 105 deg W slot, the eclipse count behind the battery cycle-life argument, the client geometry driving the mated inertia: all were used without saying where they came from. The fix that scales is an **assumptions register** (Appendix B of the final report): Parameter, Value, Basis, What it drives, If it is wrong. The last column is what makes it an engineering artifact instead of a disclaimer.

**Redundancy audit method that worked:** map every line to its section, then count where each key claim or number appears. Repeated *numbers* are usually fine, because they are shared inputs feeding different calculations. Repeated *arguments* are the problem. In this report that surfaced a trade verdict stated in full twice (Section 3.2 before the evidence and Section 4.5 after it), two sections opening with near-identical sentences, and a prose section that restated what the table immediately below it already carried.
