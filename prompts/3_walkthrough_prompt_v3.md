# Socratic Solution Walkthrough Prompt — v2
...
### 7. Feynman test (Plain-English Intuition)

Every problem must end with a one-sentence "Feynman test" blockquote — a layman-readable distillation of *why this trick actually works at all*. Named after Feynman's principle that if you can't explain it in plain English, you don't understand it well enough yet. The Key Takeaway is the technical consolidation; the Feynman test is the line that survives after the math fades.

**Format** (place immediately after the Key Takeaway blockquote):

```
> **Feynman test (in plain English):** [one tight sentence — analogy or mental picture, no jargon, no formulas]
```

Two consecutive blockquotes — Key Takeaway (technical) → Feynman test (intuitive) — give the reader natural escalation from "what to remember" to "why it actually works."

**Rules:**
- **Exactly one sentence.** If you need two, the insight isn't compressed enough.
- **No jargon.** Avoid "linearization," "STM," "vis-viva," "anomaly," "Keplerian," "perigee," etc. A non-engineer should follow it.
- **No formulas, no Greek letters.** Use plain words: "speed," "altitude," "push," "tip," "rotate," "swap."
- **Analogies and mental pictures are preferred** — clocks running at different speeds, a leaky bucket draining, throwing away one velocity for another, pushing a swing.

**Examples (GOOD):**
- HCW relative motion: *"If they're close enough, the nonlinear forces are almost the same for both, so the difference between them behaves linearly."*
- Plane change: *"A plane change doesn't make the satellite go any faster — it just rotates which way it's pointing, and rotating the full velocity vector costs the full velocity, not a small piece of it."*
- Phase rate: *"Two satellites at different altitudes are like two clocks running at slightly different speeds — phase rate is just how fast the faster one pulls ahead per unit time."*
- Linear closure: *"If the closing rate stays constant, time-to-close is just 'angle divided by rate' — same arithmetic as a leaky bucket draining at a constant flow."*

**Anti-patterns (BAD):**
- *"The HCW STM is a Taylor expansion of the relative-motion ODE around the chief's circular orbit."* (jargon)
- *"HCW works because of linearization. The nonlinear gravity terms cancel."* (two sentences, still jargon)
- *"The plane-change cost is $2V\sin(\theta/2)$."* (just restates the formula — that's the Key Takeaway's job)
- *"This problem teaches you about plane changes."* (meta — describes pedagogy, not mechanism)

The Feynman test is the highest-compression layer of the walkthrough. Each one should survive the rest of the document fading from memory and still leave the reader with the right intuition.
...
**Problem Statement:** [Brief restatement, listing every sub-part (a), (b), (c) ... verbatim]

**The punchline first:** [1-2 sentences: the answer and the key idea]

[If the problem has 2+ sub-parts: insert the Part-to-Section Map table here — see
 "Part-to-section mapping" rule in the Detailed Rules section below.]

---

### N.X (part letters) [Descriptive Subsection Title]
...
[Repeat subsections as needed; tag each header with the part letter(s) it answers]

[Insert relevant figure after the section it illustrates]

> **Results for Problem N** (required whenever the problem has 2+ sub-parts)
> - **(a)** [boxed answer]
> - **(b)** [boxed answer]
> - ... one bullet per part letter

> **Key takeaway from Problem N:** [2-3 sentence consolidation]

> **Feynman test (in plain English):** [one tight layman-readable sentence — see Principle 7]
...
### Part-to-section mapping (when the problem has sub-parts)

If a problem statement lists explicit sub-parts — (a), (b), (c), (d), or similar — the walkthrough must make the mapping from problem part to walkthrough section **unmistakable**. A reader preparing for an exam should never have to scan the full problem to answer "where is the answer to part (b)?"

Apply all three layers below whenever a problem has 2+ sub-parts.

**(1) Tag every subsection header with the part letter(s) it answers** using parentheses after the section number. Prefer one part per subsection. Multiple-part tags are only allowed when a single derivation genuinely produces several answers simultaneously.

- `### 2.1 (a) Locating POGO in ECEF`
- `### 2.4 (a, b, c, d) Rise, set, and maximum elevation` — all four parts share one rotation-chain derivation, so they live together
- `### 3.2 (b) Angular separation from the dot product`

**(2) Insert a Part-to-Section Map immediately after the problem's "punchline first"** — a compact table listing each part, its headline answer, and the section number that derives it. Example (adapted from `wk11_ex3` Problem 2):

| Part | Answer | Section |
|---|---|---|
| (a) Rise / set times | 03:21:19 / 08:05:38 UTC | §2.4 |
| (b) Azimuth at rise / set | 122.647° / 36.508° | §2.4 |
| (c) Max elevation | 41.476° | §2.4 |
| (d) Azimuth at max | 75.954° | §2.4 |

The headline-answer column is short (one number + units) so the table fits on one line per part. Use "see §X.Y" if the answer is a vector or table rather than a single scalar.

**(3) End each sub-parted problem with a Results block** — a blockquote titled `> **Results for Problem N**` that restates every part letter with its final boxed answer in a compact list. Place it *immediately before* the Key Takeaway blockquote, so the reader gets the consolidated answers and the conceptual consolidation back-to-back.

```
> **Results for Problem 2**
> - **(a)** Rise 03:21:19 UTC, set 08:05:38 UTC
> - **(b)** Az at rise 122.647°, Az at set 36.508°
> - **(c)** Max elevation 41.476°
> - **(d)** Az at max 75.954°
```

**Do not sacrifice concept-organization for part-organization.** Subsections should still be organized around the physical or mathematical idea they teach (station geometry, GHA polynomial, rotation chain, event timing), not around part letters. If parts (a) and (c) both emerge from the same rotation-chain derivation, keep them in a single subsection tagged `(a, c)`. Do **not** split a single derivation into artificially separate per-part subsections just to get one subsection per part — that fragments the teaching logic and produces redundant derivations.

The part-labeling layer is a **directory** into the concept-organized subsections, not a replacement for them.

**Why this rule exists.** v1 walkthroughs organized subsections purely by concept. That respects the derivation logic but hides which subsection answers which part. On `wk11_ex3` Problem 2, all four parts (a)–(d) were bundled into a single §2.4 with no tagging or map. A student re-reading before an exam could not find part (b) without scanning the whole problem. v2 adds the mapping layer so the reader gets concept-organized content *and* a directory to navigate it.
...
2. Read the problem statement and relevant lecture materials. For every problem, note whether it has explicit sub-parts (a), (b), (c), etc. — that triggers the Part-to-section mapping rule.
3. Plan the visual aids — identify which figures will have the highest learning impact.
4. Write the 30,000-foot overview.
5. Write each problem section: punchline first, then (if sub-parts exist) the Part-to-Section Map table, then the subsections. Tag each subsection header with the part letter(s) it answers.
6. Generate all figures (write Python scripts in `figures/`, run them, verify output).
7. For every problem that has 2+ sub-parts, add a `> **Results for Problem N**` blockquote listing each part letter with its boxed answer, placed immediately before the Key Takeaway.
8. **For every problem, write a Feynman-test blockquote** — one plain-English sentence capturing the deepest "why this trick works." Place immediately after the Key Takeaway. See Principle 7 for format and anti-patterns.
9. Write the Summary: strategy recap, self-test questions, formula table with pseudocode, variables table.
10. Add new formulas to the master pseudocode file.
11. Final review: every variable defined, every formula has pseudocode, no KaTeX errors (no unicode `·` or `×` inside `$...$`), every sub-parted problem has the three mapping layers (tagged headers, Part-to-Section Map, Results blockquote), every problem has both a Key Takeaway and a Feynman test.

---

## Version

**Current:** v3 — See `CHANGELOG.md` for full history.
